...
if ( $nv_Request->get_int('save', 'post', 0)==1 ) {
    ...
    if ( empty($code) OR empty($name) ) {
        ...
        $sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_group (code, name) VALUES ( :code, :name)';
        $data_insert = array();
        ...
        $stmt = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_group SET code= :code, name = :name WHERE id =' . $id);
...
$xtpl->assign('GROUP_LIST', nv_show_group_list($code,$name));
if (!empty($error)) {
...
if ($id > 0) {
    list($_id, $_code, $_name) = $db->query('SELECT id, code, name FROM ' . NV_PREFIXLANG . '_' . $module_data . '_group where id=' . $id)->fetch(3);
    $lang_module['add_cat'] = $lang_module['edit_cat'];
